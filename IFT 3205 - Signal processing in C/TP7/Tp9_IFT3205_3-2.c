...
int main(int argc,char **argv)
 {
  ...
  // Load the signal
  float*  Signal = LoadSignalDat("SoundFile",&length); 
  float* SignalCanal=fmatrix_allocate_1d(length);
 
  // Calculate
  canal(Signal, SignalCanal, length);
  float result = calculateRSB(Signal, SignalCanal, length);

  // Show the results
  printf("\n----------------\nRSB = %f\n----------------", result);

  SaveSignalDatWav("SoundFileInCanal",SignalCanal,length, 11025.0);
  SaveSignalDat("SoundFileInCanal",SignalCanal,length);
