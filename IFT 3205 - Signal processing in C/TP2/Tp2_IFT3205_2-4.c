...
int main(int argc,char **argv)
 {
  ...
  float** MatriceImgOrg2=LoadImagePgm(NAME_IMG_IN2,&length,&width);  // Ajouté +++ 
  ...
  float result = FindAngle(MatriceImgR2, MatriceImgM2, MatriceImgM1, length, width, -M_PI/16, M_PI/16, 0.005, 0);

  //IFFTDD(MatriceImg1, MatriceImgI1, length, width);
  //IFFTDD(MatriceImg2, MatriceImgI2, length, width);             
  
  Rotate(MatriceImgR2, MatriceImgOrg2, result , length, width);
