...
void DeleteHarmAxisV(float**, int, int, int);
...
void DeleteHarmonicsB(float** matrix, int length, int width, int radius) {
  //int radius = 7;
  for(int i=0; i<length; i++) {
    for(int j=0; j<width; j++) {
      if (sqrt(pow((i - length/2), 2) + pow((j - width/2), 2)) > radius) { 
        matrix[i][j]=0.0;
      }
    }
  }
...
void AmplifyingHarmonics(float** matrix, int length, int width) {
  for(int i=0; i<length; i++) {
    for(int j=0; j<width; j++) {
      matrix[i][j] *= Distance(i, j, length, width);
    }
  }
}


void DeleteHarmAxisV(float** matrix, int length, int width, int pixelDist) {
  int halfWidth = width / 2;
  
  for(int i=0; i<length; i++) {
    for(int j=0; j<width; j++) {
      if (j > (halfWidth + pixelDist) || j < (halfWidth - pixelDist)) {
        matrix[i][j] = 0.0;
      }
    }
  }
...
void DeleteHarmAxisU(float** matrix, int length, int width, int pixelDist) {
  int halfLength = length / 2;

  for(int i=0; i<length; i++) {
    for(int j=0; j<width; j++) {
      if (i > (halfLength + pixelDist) || i < (halfLength - pixelDist)) {
        matrix[i][j] = 0.0;
      }
    }
  }
...
int main(int argc,char **argv)
 {
  ...
  /*Find the frequency operation that enables us to obtain the image*/
  /*for (int i = 0; i < length; i++) {
    for (int j = 0; j < width; j++) {
  ...
  DeleteHarmonicsB(MatriceImgR, length, width, 19);  
  DeleteHarmonicsB(MatriceImgI, length, width, 19);
