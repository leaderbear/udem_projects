...
void copySectionMatrix(float** sourceMatrix, float** targetMatrix, int option, int startRow, int startCol, int numRows, int numCols) {
    ...
    for (i = 0; i < blockSize; i++) {
        for (j = 0; j < blockSize; j++) {
            if (option) {targetMatrix[i][j] = sourceMatrix[(i + startRow) % numRows][(j + startCol) % numCols];} 
            else {targetMatrix[(i + startRow) % numRows][(j + startCol) % numCols] = sourceMatrix[i][j];}
        }
    }
}
...
void denoise(float **target_matrix, float **matrix, float **bloc_8x8, int length, int width, int n, int offset_i, int offset_j, int option)
{
  ...
  for (int i = offset_i; i < length; i += iter)
  {
    for (int j = offset_j; j < width; j += iter)
    {
      ...
      else { cancelCoefficients_T(bloc_8x8, n); } // For 2-4 (later)
      ddct8x8s(1, bloc_8x8);
...
//This function denoises an image by averaging the denoised image resulting from different shifts of the blocks
void denoiseAverage(float** target_matrix, float*** matrix_3d, float** mat, float** img, float** bloc_8x8, int length, int width, int n){
  //denoise each image for each combination of shifts
  int count = 0;
  for(int i = 0; i < blockSize ; i++){
    for(int j = 0; j < blockSize; j++){
      denoise(target_matrix, mat, bloc_8x8, length, width, n, i, j, 0);
      copy_matrix(matrix_3d[count++], target_matrix, length, width);
    }
  }

  //average the denoised images
  float average;
  for(int i = 0; i < length; i++){
    for(int j = 0; j < width; j++){
      average = 0;
      for(int k = 0; k < num_shifts; k++){
          average += matrix_3d[k][i][j];
        }
      target_matrix[i][j] = average / num_shifts;
    }
  }
}
...
int findbestThreshold_T2(float** result, float*** matrix_3d, float** mat, float** img, float** bloc_8x8, int length, int width, int offset_i ,int offset_j ,int option, int max_iter){
  int best_treshold = 0;
  float min_mse = INFINITY;
  float mse;
  int t;
  ...
  for(t = 0; t < max_iter; t++){
    ...
    // denois (& average) the image with t treshold
    if (option){denoise(result, mat, bloc_8x8, length, width, t, offset_i, offset_j, 0);} 
    else{denoiseAverage(result, matrix_3d, mat, img, bloc_8x8, length, width, t);}  
  ...
  // Denoise the image with the best treshold
  denoiseAverage(result, matrix_3d, mat, img, bloc_8x8, length, width, best_treshold);
...
int main(int argc,char** argv)
{
 ...
 char BufSystVisuImg[NBCHAR];
  ...
  float*** matrix_3d=fmatrix_allocate_3d(64,length,width);  
 ...
 //=========================================================
  ...
  int best_t = findbestThreshold_T2(ImgDenoised, matrix_3d, ImgDegraded, Img, bloc_8x8, length, width, 0, 0, 0, 100); 
  printf("\n------------\n> Best T = %d\n------------",best_t);
