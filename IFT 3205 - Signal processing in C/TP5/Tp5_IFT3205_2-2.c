...
void denoise(float** target_matrix, float** matrix, float** bloc_8x8, int length, int width, int n, int offset_i, int offset_j, int option) {
    ...
    for (int i = offset_i; i < length; i += iter) {
        for (int j = offset_j; j < width; j += iter) {
            ...
            else{/*cancelCoefficients_T*/}                        // For 2-4 (later)
            ddct8x8s(1, bloc_8x8);
...
int findbestThreshold_N(float** result, float** mat, float** img, float** bloc_8x8, int length, int width, int offset_i ,int offset_j , int max_iter){
  int best_treshold = 0;
  ...
  for(n = 0; n < max_iter; n++){
    ...
    denoise(result, mat, bloc_8x8, length, width, n,offset_i , offset_j, 1);
  ...
  denoise(result, mat, bloc_8x8, length, width, best_treshold,  offset_i , offset_j, 1);
...
int main(int argc,char** argv)
{
 ...
 int best_n = findbestThreshold_N(ImgDenoised, ImgDegraded, Img, bloc_8x8, length, width, 0, 0, 64); 
 printf("\n------------\n> Best N = %d\n------------",best_n);
