...
int main(int argc,char **argv)
 {
  ...
  calculateAveragedPeriodogramV2(Sign1, Samples, SamplesI, SampleAverages, SampleAveragesI, length - step, step, sample_size);

  ModVct(SampleAveragesM, SampleAverages, SampleAveragesI, sample_size);
  CenterVct(SampleAveragesM, sample_size);

  // Sauvegarde
  SaveSignalDat("FFT_Moteur2_PermMoyHann", SampleAveragesM, sample_size);
