...
public class JavaFiles {
     ...
     * @throws IOException
     */
    public File writeDataToCsv(String filePath, String header, boolean writeConsole) throws IOException {
        File csvFile = new File(filePath);
        FileWriter fileWriter = new FileWriter(csvFile);

        //StringBuilder header = new StringBuilder();
        //header.append("location,package,name,commentsDensity\n");
        fileWriter.write(header.toString());

        // TODO : cette fonction va à partir de maintenant seulement considérer la métrique commentsDensity , effacer tout le reste
        for (JavaFile jf : this.list) {
            ...
            if (jf.getCloc() != -1) {
                line.append("," + jf.getCloc());
            }
            if (jf.getCommentsDensity() != -1.0) {
                line.append("," + jf.getCommentsDensity());
            }
            if (jf.getLastCommit() != null){
                SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
                line.append("," + sdf.format(jf.getLastCommit()));
            }
