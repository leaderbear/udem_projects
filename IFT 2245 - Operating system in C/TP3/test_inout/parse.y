...
int main (int argc, char *argv[])
{
  ...
  /* Charlie 2020: wow such savings of the wows pm */
  int count_backupped = 0;
  int count_faulted = 0;
  for(int i=0; i<256; i++) {
    int frame = pt_lookup(i);
    if(frame < 0) {
        count_faulted++;
        continue;
    }

    //frame is thus valid. Now, we need to save it to the store
    pm_backup_page(frame, i);
    count_backupped++;
  }
  /* Charlie 2020: wow such savings of the wows pm END */
  ...
  fflush(stdout);
    ...
    fprintf(stdout, "calling the fixer now\n");
  /* Charlie 2020: wow such savings of the wows pm START */
  char call_str[100];
  sprintf(call_str, "python3 ./report_fixer.py %d", count_faulted);
  system(call_str);
  /* Charlie 2020: wow such savings of the wows pm END */
