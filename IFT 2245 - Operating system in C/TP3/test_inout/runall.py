...
def _1():
    ...
    _, report, _ = run("../../tests/command_tlb.in")
...
def _2():
    ...
    _, report, _ = run("../../tests/command_pt.in")
...
def scale(real, best, mid, worst):
    ...
    if real <= mid:
        ...
        return abs((real / best) / 4 + 0.5)

    if real <= worst:
        real -= mid
        worst -= mid

        return abs((1 - real / worst) * 0.5)
...
def _5():
    ...
    for file in os.listdir("cmds"):
        with directory("tests_build"):
            ...
            success_pa = True
            for co, ct in zip(o_calls, t_calls):
                if co.pa != ct.pa:
                    success_pa = False
            if success_pa:
                successes_pa += 1
            if o_report['pmreads'] == t_report['pmreads']:
                successes_pm_reads += 1



    score = successes_bs/nb_tests + successes_pm_reads/nb_tests  #successes_pa/nb_tests +
    return good(score/2 * 20)
try:
    _1()
    _2()
except:
...
with directory("../test_check"):
    ...
    print("SUBGRADE: ", float(out2)*3*20)
    subgrade = float(out2)*3*20

print(f"GRADE:{{{(subgrade + pts_lost)/60}}}")
