  ...
  for (int i = 0; i < TLB_NUM_ENTRIES; i++) {
    if (tlb_entries[i].frame_number >= 0 && tlb_entries[i].page_number == page_number) {
      if (write && tlb_entries[i].readonly) {
        return -1; // Accès en écriture non autorisé
      } else {
                            ...
                            unsigned int frame_number, bool readonly)
{
  ...
  for (int i = 0; i < TLB_NUM_ENTRIES; i++) {
    ...
    if (tlb_entries[i].page_number < tlb_entries[oldest_entry_index].page_number) {
      oldest_entry_index = i;
    }
  }
  tlb_entries[oldest_entry_index].page_number = page_number;
  tlb_entries[oldest_entry_index].frame_number = frame_number;
  tlb_entries[oldest_entry_index].readonly = readonly;
}
